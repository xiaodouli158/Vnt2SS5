...
pub fn init_proxy(
    ...
    let proxy_map = if tokio::runtime::Handle::try_current().is_ok() {
        // We're already in a Tokio runtime, so we can just spawn a task
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(init_proxy0(context, current_device, client_cipher))
        })?
    } else {
        ...
        let runtime = tokio::runtime::Builder::new_multi_thread()
            ...
            .thread_name("ipProxy")
            .build()?;
        runtime.block_on(init_proxy0(context, current_device, client_cipher))?
    };
