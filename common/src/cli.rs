...
pub fn parse_args_config() -> anyhow::Result<Option<(Config, Vec<String>, bool)>> {
    ...
    opts.optflag("", "ss5", "启用SOCKS5服务");
    opts.optopt("", "ss5port", "SOCKS5服务端口(默认10080)", "<port>");
    opts.optflag("", "ss5log", "显示SOCKS5日志");
    ...
    let (config, vnt_link_config, cmd) = if conf.is_some() {
        ...
        // SOCKS5 options
        let enable_socks5 = matches.opt_present("ss5");
        let socks5_port = matches
            .opt_get::<u16>("ss5port")
            .unwrap_or(None)
            .unwrap_or(10080);
        let socks5_logging = matches.opt_present("ss5log");
